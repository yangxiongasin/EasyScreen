...
const checkCode = (response) => {
  ...
  if (response.data.code) { // 业务接口错误
    if (response.data.code !== 200) { // 接口错误
...
http.$qmc.interceptors.request.use(requestInterceptor, requestError)
http.$http.interceptors.response.use(responseInterceptor, responseError)
http.$qmc.interceptors.response.use(responseInterceptor, responseError)
